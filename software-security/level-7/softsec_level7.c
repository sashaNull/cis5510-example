...
void auth(int pennkey, char *name, char *email, char *research, int course_code,
          char *cmd, bool pwnnsylvanian, char *fav_ctf_category) {
...
int main() {
  setuid(0);
  auth(0x1337, "sebastian", "sebastian.angel@cis.upenn.edu", "Cybersecurity",
       5510, "echo CIS5510 is awesome!", true, "pwn, rev");
  read_buffer();
  return 0;
}
